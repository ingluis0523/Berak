...
export default function MinisteriosClient({ canCrear, canEditar }: Props) {
  ...
  const fetchMinisterios = useCallback(async () => {
    ...
    const { data: pm } = await supabase.from('persona_ministerios').select('ministerio_id').eq('activo', true)
    const countMap: Record<string, number> = {}
    pm?.forEach((r) => { if (r.ministerio_id) countMap[r.ministerio_id] = (countMap[r.ministerio_id] ?? 0) + 1 })
  ...
  function openEdit(m: Ministerio) {
    setEditing(m)
    setForm({ nombre: m.nombre, descripcion: m.descripcion ?? '', lider_id: m.lider_id ?? '' })
    setFormErrors({})
    setModalOpen(true)
  }
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent size="md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing ? 'Modifica los datos del ministerio.' : 'Completa los datos para crear un nuevo ministerio.'}
            </DialogDescription>
          </DialogHeader>
          <div className="px-6 py-4 space-y-4">
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700">Nombre <span className="text-red-500">*</span></label>
              <Input value={form.nombre} onChange={(e) => setForm((f) => ({ ...f, nombre: e.target.value }))} placeholder="Ej: Ministerio de Alabanza" />
            ...
            <div className="space-y-1">
              ...
              <Textarea value={form.descripcion} onChange={(e) => setForm((f) => ({ ...f, descripcion: e.target.value }))} placeholder="Descripción del ministerio..." rows={3} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setModalOpen(false)} disabled={saving}>Cancelar</Button>
            <Button onClick={handleSave} loading={saving}>{editing ? 'Guardar cambios' : 'Crear ministerio'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
