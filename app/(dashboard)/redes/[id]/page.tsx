...
export default async function RedDetailPage({ params }: PageProps) {
  ...
  const hasFullAccess =
    currentUser?.is_admin || currentUser?.hasPermission("acceso_todas_redes");
  const canEditar = currentUser?.hasPermission("editar_redes") ?? true;

  const now = new Date();
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);

  const [{ data: red }, { data: grupos }, { data: estadoNuevoRow }] =
    await Promise.all([
      supabase
        .from("redes")
      ...
      supabase
        .from("grupos")
        .select(
          "id, nombre, estado, lider:personas!lider_id(id, nombres, apellidos)",
        )
        .eq("red_id", id)
        .is("deleted_at", null)
        .order("nombre"),
  ...
  if (grupoIds.length > 0) {
    const { data: miembros } = await supabase
      .from("grupo_miembros")
      .select(
        "persona:personas(id, nombres, apellidos, tipo_persona, estado_persona_id, fecha_registro), grupo:grupos(nombre, id)",
      )
    ...
    for (const m of miembros ?? []) {
      ...
      if (p && !seen.has(p.id)) {
        ...
        personas.push({
          ...
          estado_persona_id: p.estado_persona_id,
  ...
  const lider = (Array.isArray(liderRaw) ? liderRaw[0] : liderRaw) as Pick<
    Persona,
  ...
  for (const g of grupos ?? []) {
    ...
    const lG = (Array.isArray(lGRaw) ? lGRaw[0] : lGRaw) as Pick<
      Persona,
      "id" | "nombres" | "apellidos"
    > | null;
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            {red.descripcion && (
              <p className="text-sm text-gray-500 mt-1">{red.descripcion}</p>
            )}
            <p className="text-xs text-gray-400 mt-1">
              Creada {formatDate(red.created_at)}
            </p>
          </div>
                ...
                {grupos.map((g) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Badge
                        variant={g.estado ? "success" : "inactivo"}
      ...
      <Card>
        ...
        <CardContent>
          {personas.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100 max-h-120 overflow-y-auto -mx-6 px-6">
              {personas.map((p) => (
