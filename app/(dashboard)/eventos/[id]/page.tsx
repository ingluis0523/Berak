...
export default async function EventoDetallePage({
  ...
  if (grupoContexto && memberIdsForFilter && memberIdsForFilter.length > 0) {
    const recordedIds = new Set(
      asist
        .filter((a) => a.persona_id && !a.es_visitante)
        .map((a) => a.persona_id!),
    );
  ...
  if (grupoContexto) {
    ...
    const uniqueIds = new Set(
      (allMembers ?? []).map((m) => m.persona_id as string),
    );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {ev.grupo && <Badge variant="secondary">{ev.grupo.nombre}</Badge>}
          </div>
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs text-gray-500">
                <span>
                  {asistio.length} de {totalMiembros}{" "}
                  {ev.grupo_id ? "miembros" : "registros"} asistieron
                </span>
                <span>{pct}%</span>
              </div>
                  ...
                  <ul className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
                    {asistio.map((a) => {
                      ...
                      const nombre = p
                        ...
                        : (a.nombre_visitante ?? "Persona");
                      return (
                        <li
                          key={a.id}
                          className="flex items-center gap-3 px-5 py-3 bg-green-50/30"
                        >
                  ...
                  {visitantes.map((a) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge
                          variant={
                            a.estado === "primera_vez" ? "warning" : "visitante"
                          }
                        >
