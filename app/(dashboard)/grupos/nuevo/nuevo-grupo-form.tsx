...
export default function NuevoGrupoForm({ personas, redes, defaultRedId, lockRed }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.lider_id) updates.push(Promise.resolve(supabase.from('personas').update({ tipo_persona: 'lider' }).eq('id', form.lider_id)))
    if (form.sublider_id && form.sublider_id !== 'none') updates.push(Promise.resolve(supabase.from('personas').update({ tipo_persona: 'sublider' }).eq('id', form.sublider_id)))
    if (form.anfitrion_id && form.anfitrion_id !== 'none') updates.push(Promise.resolve(supabase.from('personas').update({ tipo_persona: 'anfitrion' }).eq('id', form.anfitrion_id)))
  ...
  const redNombreLocked = lockRed ? (redes.find((r) => r.id === defaultRedId)?.nombre ?? null) : null
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-end gap-3 pb-6">
          <Button type="button" variant="outline" onClick={() => router.push('/grupos')} disabled={saving}>
            Cancelar
          </Button>
